...
laptop_type = st.selectbox("Type of laptop",df['TypeName'].unique(),index=1)
cpu = st.selectbox("Processor",df['Cpu'].unique(),index=1)
ram = st.radio("RAM(in GB)",[4,6,8,12,16,24,32,64,128], index=4,horizontal=True)
...
touchscreen = st.radio("Touch Screen?",['Yes','No'],horizontal=True)
cpu_speed = st.slider("CPU clock Speed",min_value=0.9,max_value=3.6,value=2.4,step=0.1)
...
if st.button("PREDICT PRICE"):
    ...
    query = np.array([[company,laptop_type,cpu,ram,gpu,os,weight,ips,touchscreen,cpu_speed,hdd,ssd,ppi]])
